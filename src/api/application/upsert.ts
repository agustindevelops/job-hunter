...
/** Syncs children + optional cover letter for an application. Call inside a transaction. */
export async function syncApplicationContent(
  ...
  await cascadeById({
    ...
    updateOne: async (id, item) => {
      ...
      await db.experiences.update(id, { ...rest, applicationId });
      await setEntityTags(db.experienceTags, "experienceId", id, tags ?? []);
    },
    createOne: async (item) => {
      const { id: _id, tags, ...rest } = item;
      const newId = requireId(
        await db.experiences.add({ ...rest, applicationId }),
        "experience",
      );
  ...
  await cascadeById({
    ...
    deleteOne: async (id) => {
      await db.skillCategoryTags.where("skillCategoryId").equals(id).delete();
    ...
    createOne: async (item) => {
      const { id: _id, tags, ...rest } = item;
      const newId = requireId(
        await db.skillCategories.add({ ...rest, applicationId }),
        "skillCategory",
      );
      await setEntityTags(
        db.skillCategoryTags,
  ...
  await cascadeById({
    ...
    updateOne: async (id, item) => {
      ...
      await setEntityTags(db.achievementTags, "achievementId", id, tags ?? []);
    },
    createOne: async (item) => {
      const { id: _id, tags, ...rest } = item;
      const newId = requireId(
...
/** Updates a tailored (or any) application tree by id. */
export async function upsertApplication(
  applicationId: number,
  input: UpsertApplicationInput,
): Promise<void> {
  const app = await db.applications.get(applicationId);
  if (!app?.id) throw new Error(`Application ${applicationId} not found`);
  ...
  await db.transaction(
    ...
    async () => {
      await syncApplicationContent(applicationId, input);
    },
  );
}
